...
public class MessageEndpointTest implements TestData {
    ...
    public void beforeEach() {
        userRepository.saveUser(TestData.buildApplicationUserWith(ADMIN_USER));
    }

    @AfterEach
    public void afterEach() {
        userRepository.deleteUser(userRepository.findUserByEmail(ADMIN_USER));
    }
    ...
    public void givenNothing_whenFindAll_thenEmptyList() throws Exception {
        ...
        CustomPage<SimpleMessageDto> page = objectMapper.readValue(response.getContentAsString(), new TypeReference<>() {});
        List<SimpleMessageDto> simpleMessageDtos = page.getContent();
    ...
    public void givenOneMessage_whenFindAll_thenListWithSizeOneAndMessageWithAllPropertiesExceptSummary()
        ...
        assertAll(
            () -> assertEquals(savedMessage.getId(), simpleMessageDto.getId()),
    ...
    public void givenOneMessage_whenFindById_thenMessageWithAllProperties() throws Exception {
        ...
        MvcResult mvcResult = this.mockMvc.perform(get(MESSAGE_BASE_URI + "/{id}", savedMessage.getId())
            .header(securityProperties.getAuthHeader(), jwtTokenizer.getAuthToken(ADMIN_USER, ADMIN_ROLES)))
            .andDo(print())
            .andReturn();
        MockHttpServletResponse response = mvcResult.getResponse();
        ...
        DetailedMessageDto detailedMessageDto = objectMapper.readValue(response.getContentAsString(), DetailedMessageDto.class);


        assertEquals(savedMessage, messageMapper.detailedMessageDtoToMessage(detailedMessageDto));
    }
    ...
    public void givenNothing_whenPost_thenMessageWithAllSetPropertiesPlusIdAndPublishedDate() throws Exception {
        ...
        assertAll(
            () -> assertEquals(message.getTitle(), messageResponse.getTitle()),
            () -> assertEquals(message.getSummary(), messageResponse.getSummary()),
            () -> assertEquals(message.getText(), messageResponse.getText())
        );
    }
